...
A stream's log is read from the **stream's own shard**, not the consumer's. History nodes are stored per shard, so an external stream read from the consumer's shard returns no nodes at all. That surfaces as `Workflow execution history not found` from the workflow task start, which matching logs as a task it could not start and drops, so the symptom is a workflow that never receives its task rather than an error anyone sees. The shard is derived from the stream id through `Config.GetShardID`, the same mapping the rest of the server uses.
